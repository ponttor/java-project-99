...
class TasksControllerTest {
    ...
    void shouldCreateTask() throws Exception {
        var request = Map.of("index", 12, "assignee_id", assignee.getId(), "title", "Test title", "content",
                "Test content", "status", taskStatus.getSlug());

        mockMvc.perform(
                post("/api/tasks").contentType("application/json").content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.id").exists())
                .andExpect(jsonPath("$.index").value(12)).andExpect(jsonPath("$.assignee_id").value(assignee.getId()))
                .andExpect(jsonPath("$.title").value("Test title"))
                .andExpect(jsonPath("$.content").value("Test content")).andExpect(jsonPath("$.status").value("draft"))
    ...
    void shouldRejectMissingTaskLabel() throws Exception {
        var request = Map.of("title", "Task", "status", taskStatus.getSlug(), "taskLabelIds", List.of(999));
    ...
    void shouldShowTask() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks/{id}", task.getId())).andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.title").value("Task 1")).andExpect(jsonPath("$.content").value("Description"))
                .andExpect(jsonPath("$.status").value("draft")).andExpect(jsonPath("$.createdAt").exists());
    }
    ...
    void shouldListTasks() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks")).andExpect(status().isOk()).andExpect(header().string("X-Total-Count", "2"))
                ...
                .andExpect(jsonPath("$[1].title").value("Task 2")).andExpect(jsonPath("$[1].assignee_id").isEmpty());
    }
    ...
    void shouldFilterTasksByAssignee() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks").queryParam("assigneeId", assignee.getId().toString()))
                ...
                .andExpect(jsonPath("$", hasSize(1))).andExpect(jsonPath("$[0].title").value("First task"));
    }
    ...
    void shouldCombineAllTaskFilters() throws Exception {
        ...
        mockMvc.perform(get("/api/tasks").queryParam("titleCont", "create")
                .queryParam("assigneeId", assignee.getId().toString()).queryParam("status", "published")
                .queryParam("labelId", bug.getId().toString())).andExpect(status().isOk())
                .andExpect(header().string("X-Total-Count", "1")).andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].title").value("Create new version"));
    }
    ...
    void shouldRejectInvalidTaskData() throws Exception {
        var request = Map.of("title", "", "status", taskStatus.getSlug());
        ...
        mockMvc.perform(post("/api/tasks").contentType("application/json").content("{\"title\":\"Task\"}"))
                .andExpect(status().isBadRequest());
        mockMvc.perform(
                post("/api/tasks").contentType("application/json").content("{\"title\":\"Task\",\"status\":null}"))
                .andExpect(status().isBadRequest());
    }
    ...
    void shouldReturnNotFoundForMissingTaskRelations() throws Exception {
        var missingStatusRequest = Map.of("title", "Task", "status", "missing");
        var missingAssigneeRequest = Map.of("title", "Task", "status", taskStatus.getSlug(), "assignee_id", 999);

        mockMvc.perform(post("/api/tasks").contentType("application/json")
                .content(objectMapper.writeValueAsString(missingStatusRequest))).andExpect(status().isNotFound());
        mockMvc.perform(post("/api/tasks").contentType("application/json")
                .content(objectMapper.writeValueAsString(missingAssigneeRequest))).andExpect(status().isNotFound());
    }
    ...
    void shouldAcceptLongTitleAndContent() throws Exception {
        ...
        var request = Map.of("title", longTitle, "content", longContent, "status", taskStatus.getSlug());

        mockMvc.perform(
                post("/api/tasks").contentType("application/json").content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.title").value(longTitle))
                .andExpect(jsonPath("$.content").value(longContent));
    }

    @Test
    void shouldCreateTaskWithoutOptionalFields() throws Exception {
        var request = Map.of("title", "Minimal task", "status", taskStatus.getSlug());
