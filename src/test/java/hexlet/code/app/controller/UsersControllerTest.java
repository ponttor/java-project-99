...
class UsersControllerTest {
    ...
    void shouldCreateUser() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.firstName").value("Jack")).andExpect(jsonPath("$.lastName").value("Jons"))
                .andExpect(jsonPath("$.createdAt").exists()).andExpect(jsonPath("$.password").doesNotExist());
    ...
    void shouldUpdateUserPartially() throws Exception {
        ...
        mockMvc.perform(put("/api/users/{id}", user.getId()).contentType("application/json")
                ...
                .andExpect(jsonPath("$.id").value(user.getId())).andExpect(jsonPath("$.email").value("jack@yahoo.com"))
                .andExpect(jsonPath("$.firstName").value("Jack")).andExpect(jsonPath("$.lastName").value("Jons"))
                .andExpect(jsonPath("$.password").doesNotExist());
    ...
    void shouldInitializeAdminUser() {
        ...
        var admin = userRepository.findAll().stream().filter(user -> user.getEmail().equals("hexlet@example.com"))
